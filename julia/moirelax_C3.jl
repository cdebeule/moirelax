...
function ElasticitySolver(M::Matrix{Float64}, N_shell::Integer=4)
    ...
    # M can be used to switch between g and b: g = M^T b : so g. g = b (M M^T). b
    # we define a normalized M to make the code more efficient
...
function get_U(
    ...
    println("|====================MoireRelax===================|")
    println("|=================================================|")
    println(@sprintf("| θ       :%30s    deg  |", θ))
    println(@sprintf("| λ1      :%30s         |", λ[1]))
    println(@sprintf("| λ2      :%30s         |", λ[2]))
    println(@sprintf("| μ1      :%30s         |", μ[1]))
    println(@sprintf("| μ2      :%30s         |", μ[2]))
...
# grid in q = a^2 / (c1 L^2)
var = reverse(5:1.0:6)

for (i, q) in enumerate(var)

    # Vg is defined here with an extra factor (L / a)^2
    Vg = [1 / q; 0; 0; 0; 0]
